USE WAREHOUSE COMPUTE_WH; --SET YOUR WAREHOUSE

USE DATABASE DEV_PAUL; --SET YOUR DEFAULT DATABASE. THIS IS WHERE THE TABLES AND VIEWS WILL BE WRITTEN TO. THIS DATABASE SHOULD BE THE SAME ONE WHERE YOUR TUVA PROJECT TABLES ARE LOCATED UNLESS YOU EXPLICITELY POINT THE QUERIES BELOW TO READ FROM A DIFFERENT DATABASE


CREATE OR REPLACE SCHEMA OUTLIERS; --CREATE A NEW SCHEMA CALLED "OUTLIERS" AS THE DESTINATION FOR THE OUTPUT TABLES CREATED BY THE QUERY

USE SCHEMA OUTLIERS; --SET OUTLIERS AS THE DEFAULT DATABASE (OPTIONAL)


--MEMBER_MONTHS QUERY PULLING IN DEMOGRAPHIC DATA AND RISK SCORE DATA INTO A TABLE WITH ONE ROW PER MEMBER PER MONTH 
...
--QUERY PULLING SELECT DIMENSIONS FROM MEDICAL CLAIMS FOR SPECIFIED PERIOD
...
--QUERY PULLING SELECT DIMENSIONS FROM PHARMACY CLAIMS FOR SPECIFIED PERIOD
...
-- QUERY COMBINING MEDICAL AND PHARMACY CLAIMS INTO ONE TABLE. ALSO SETS SERVICE CATEGORY AND ENCOUNTER TYPES FOR PHARMACY CLAIMS TO "PHARMACY" AND USES THE CLAIM_ID FOR PHARMACY CLAIMS AS THE ENCOUNTER ID SO PHARMACY CLAIMS GET COUNTED AS UTILIZATION. 
...
--AGGREGATE CLAIMS TO REDUCE ROW COUNT BY DROPPING LINE NUMBER AND DATE FIELDS
...
--IDENTIFY OUTLIER MEMBERS

CREATE OR REPLACE VIEW OUTLIERS.OUTLIER_MEMBERS AS
WITH MEMBER_PAID_BY_YEAR AS (  --CTE TO GET TOTAL PAID AMOUNT BY MEMBER FOR OUTLIER IDENTIFICATION
  SELECT 
    MEMBER_ID, 
    INCR_YEAR, 
...
STATS_BY_YEAR AS (
  SELECT                       --CTE TO GET AGGREGATE STATISTICS FOR THE ENTIRE POPULATION BY YEAR (FOR COST OVERVIEW PAGE)
    INCR_YEAR,
...
SELECT                  --QUERY DEFINING OUTLIER THRESHOULD (MEAN + 2*STD. DEVIATION) AND FLAGGING MEMBERS WITH TOTAL COST EXCEEDING THAT AMOUNT AS OUTLIERS
  mpby.MEMBER_ID,
  mpby.INCR_YEAR,
  mpby.PAID,
  sby.TOTAL_MEMBERS,
...
--FILTER ALL_CLAIMS_AGG TO ONLY OUTLIER MEMBERS (EXPORT THIS AS A CSV CALLED: "outlier_claims_agg.csv" and put it in streamlit/streamlit_apps/outlier_analysis/data)
...
--FILTER MEMBER_MONTHS TO ONLY OUTLIER MEMBERS (EXPORT THIS AS A CSV CALLED: "outlier_member_months.csv" and put it in streamlit/streamlit_apps/outlier_analysis/data)
